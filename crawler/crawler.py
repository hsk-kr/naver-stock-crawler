...
from lib.stock_analyzer import (
    analyze_stock_trading_trend,
    calculate_analyzed_data_probability,
)
...
if __name__ == "__main__":
    ...
    api_log(
        crawler,
        "Start. [{0}]-[{1}]".format(
            "now" if start_date == None else start_date, end_date
        ),
    )
    ...
    if start_date == None:
        ...
        kospi_stocks = obtain_trading_trend_from_stocks(
            kospi_stocks, start_date, end_date
        )
    ...
    if start_date == None:
        kosdaq_stocks = obtain_trading_trend_from_stocks(
            kosdaq_stocks, end_date=end_date
        )
    else:
        kosdaq_stocks = obtain_trading_trend_from_stocks(
            kosdaq_stocks, start_date, end_date
        )
